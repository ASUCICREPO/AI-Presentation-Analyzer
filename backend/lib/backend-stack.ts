...
export class AIPresentationCoachStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: AIPresentationCoachStackProps) {
    ...
    // Handles ALL persona-related logic: resolving
    // single/multi persona configs into merged best
    // practices & scoring weights, generating unified
    // prompts for post-meeting analytics (calls Bedrock
    // to merge multiple personas), and persona
    // customization text upload/read with S3.
    // ──────────────────────────────────────────────
    const multiPersonaResolverLambda = new lambda.Function(this, 'MultiPersonaResolverLambda', {
      runtime: lambda.Runtime.PYTHON_3_13,
      handler: 'index.lambda_handler',
      code: lambda.Code.fromAsset(path.join(__dirname, '..', 'lambda', 'persona-resolver')),
    ...
    // Grant Bedrock InvokeModel for merging multiple personas
    ...
    const postMeetingAnalyticsLambda = new lambda.Function(this, 'PostMeetingAnalyticsLambda', {
      ...
      environment: {
        'UPLOADS_BUCKET': presentationAndSessionUploadsBucket.bucketName,
        'PERSONA_RESOLVER_FUNCTION': multiPersonaResolverLambda.functionName,
      },
    });
    ...
    // Grant post-meeting analytics permission to invoke the persona resolver
    multiPersonaResolverLambda.grantInvoke(postMeetingAnalyticsLambda);
    ...
    NagSuppressions.addResourceSuppressions(multiPersonaResolverLambda.role!, [
      ...
      { id: 'AwsSolutions-IAM5', reason: 'Bedrock InvokeModel wildcard allows model switching for persona merging. Cross-region inference profiles route to multiple regions.', appliesTo: ['Resource::arn:aws:bedrock:*::foundation-model/*', `Resource::arn:aws:bedrock:*:<AWS::AccountId>:inference-profile/*`] },
    ], true);
