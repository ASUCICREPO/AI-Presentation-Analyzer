...
import { Sun, Ruler, ScanFace, Mic, Check, CheckCircle2, ArrowLeft, ArrowRight, ArrowUp, ArrowDown, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="animate-fade-in flex flex-col">
      <div className="flex items-center gap-2 mb-3 border-b pb-3">
        ...
        <h3 className="font-serif text-base font-bold text-gray-900 2xl:text-xl">Calibration Check</h3>
      </div>
      ...
      <div className="space-y-4">
        ...
        {/* Mic Check */}
        <div className={`rounded-xl p-3 border-2 transition-all duration-300 ${getMicStatusColor()}`}>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 mb-2 font-sans text-center">Microphone Check</p>
          <div className="flex items-center justify-center gap-1 mb-2" role="meter" aria-label="Microphone volume level" aria-valuenow={micCalibration.volume} aria-valuemin={0} aria-valuemax={100}>
            <Mic className={`w-4 h-4 mr-1 ${micCalibration.hasSpeechDetected ? 'text-green-600' : 'text-gray-400'}`} />
            {Array.from({ length: barCount }).map((_, i) => (
              <div
                key={i}
                className={`w-2.5 rounded-sm transition-all duration-75 ${i < filledBars
                    ? i >= barCount - 2
                      ? 'bg-red-500'
                      : i >= barCount - 4
                        ? 'bg-orange-400'
          ...
          <div className={`text-xs font-medium font-sans text-center ${micStatus.color}`}>
            {micStatus.text}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {[
              ...
              { id: 'mic', icon: <Mic className={`w-4 h-4 ${micCalibration.hasSpeechDetected ? 'text-green-500' : 'text-gray-400'}`} />, text: micCalibration.hasSpeechDetected ? 'Microphone working' : 'Speak to verify microphone' },
            ].map((item) => (
              <div key={item.id} className="flex items-center gap-2.5 p-2.5 rounded-lg border border-gray-200 bg-white hover:border-maroon-200 transition-colors group cursor-default">
                ...
                {item.id === 'mic' && micCalibration.hasSpeechDetected && (
      ...
      <div className="pt-3 border-t mt-4">
        <button
          onClick={onComplete}
          className="w-full rounded-lg bg-maroon px-4 py-2.5 text-sm font-bold text-white shadow-lg hover:bg-maroon-dark hover:shadow-xl transform active:scale-[0.98] transition-all flex items-center justify-center gap-2 font-sans"
        >
