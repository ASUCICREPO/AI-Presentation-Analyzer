  ...
  return (
    <div className="animate-fade-in flex flex-col h-full">
       <div className="flex items-center gap-2 mb-4 border-b pb-4">
          <div className="h-8 w-8 rounded-full bg-maroon-100 text-maroon flex items-center justify-center font-bold shrink-0 font-sans">1</div>
       ...
       <div className="flex-1 overflow-y-auto space-y-6">
         ...
         <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-100">
            ...
            <button 
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-maroon/30 focus:ring-offset-2 ${
                showMesh ? 'bg-maroon' : 'bg-gray-200'
              }`}
            >
         ...
         <div className={`rounded-xl p-5 border-2 text-center transition-all duration-300 ${
            ...
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2 font-sans">Current Gaze Status</p>
         ...
         {/* Prominent Checklist */}
         <div>
            <h4 className="font-medium text-gray-900 mb-3 flex items-center gap-2 font-sans">
            ...
            <div className="space-y-3">
              {[
                ...
                { id: 'pos', icon: <Ruler className="w-5 h-5 text-maroon" />, text: 'Camera is at eye level' },
                { id: 'clear', icon: <ScanFace className="w-5 h-5 text-maroon-400" />, text: 'Face visible (no masks/hair)' },
              ].map((item) => (
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-white hover:border-maroon-200 transition-colors group cursor-default">
                   <div className="h-8 w-8 rounded-full bg-maroon-50 flex items-center justify-center group-hover:bg-maroon-100 transition-colors">
                     {item.icon}
                   </div>
       ...
       <div className="pt-4 border-t mt-auto">
         <button 
           onClick={onComplete}
           className="w-full rounded-lg bg-maroon px-4 py-3 text-sm font-bold text-white shadow-lg hover:bg-maroon-dark hover:shadow-xl transform active:scale-[0.98] transition-all flex items-center justify-center gap-2 font-sans"
         >
